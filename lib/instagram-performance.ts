...
// Motor de busca de performance na Meta, compartilhado entre a tela de
// Performance (sob demanda, quando o usuário abre a página) e o cron que
// verifica vídeos virais em segundo plano — para não existirem duas
// implementações do mesmo fetch/parse/cache podendo divergir com o tempo.
...
export function isCacheFresh(log: PerformanceLog, now = Date.now()) {
  return Boolean(
...
export function cachedResult(log: PerformanceLog, now = Date.now()): PerformanceResult {
  return {
    id: log.id,
    mediaId: log.mediaId,
...
export async function refreshPerformanceLogs(
  logs: PerformanceLog[],
  now = Date.now()
