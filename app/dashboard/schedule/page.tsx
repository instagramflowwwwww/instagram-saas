...
export default function SchedulePage() {
  ...
  useEffect(() => { sharedCoverRef.current = sharedCover }, [sharedCover])
  useEffect(() => { perVideoCoversRef.current = perVideoCovers }, [perVideoCovers])

  useEffect(() => {
    return () => {
      if (sharedCoverRef.current?.preview) URL.revokeObjectURL(sharedCoverRef.current.preview)
      Object.values(perVideoCoversRef.current).forEach((item) => URL.revokeObjectURL(item.preview))
    }
  }, [])
  ...
  const selectedItems = useMemo(
    () => selectedMedia.map((id) => media.find((item) => item.id === id)).filter((item): item is MediaItem => Boolean(item)),
    [media, selectedMedia]
  )

  const videoItems = useMemo(() => selectedItems.filter((item) => item.type === "video"), [selectedItems])
  ...
  useEffect(() => {
    setPerVideoCovers((current) => {
      ...
      Object.entries(current).forEach(([mediaId, upload]) => {
        if (allowedIds.has(mediaId)) { next[mediaId] = upload } else { changed = true; URL.revokeObjectURL(upload.preview) }
      })
      return changed ? next : current
    })
    if (videoItems.length === 0) {
      setCoverMode("none")
      if (sharedCover) { URL.revokeObjectURL(sharedCover.preview); setSharedCover(null) }
    }
  ...
  const toggleMedia = (id: string) => {
    setSelectedMedia((current) =>
      current.includes(id) ? current.filter((itemId) => itemId !== id) : current.length >= 50 ? current : [...current, id]
    )
  }
  ...
  const updatePerMedia = (mediaId: string, field: keyof CaptionDraft, value: string) => {
    setPerMedia((current) => ({
      ...current,
      [mediaId]: { caption: current[mediaId]?.caption || "", hashtags: current[mediaId]?.hashtags || "", [field]: value },
    }))
  }

  const updateRotation = (index: number, field: keyof CaptionDraft, value: string) => {
    setRotationCaptions((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, [field]: value } : entry))
  }

  const validateCoverFile = (file: File) => {
    if (file.type !== "image/jpeg") throw new Error("A capa do Reel precisa ser uma imagem JPEG.")
    if (file.size > IMAGE_LIMIT) throw new Error("A capa do Reel pode ter no máximo 8 MB.")
  }
  ...
  const handleSharedCoverChange = (file: File | null) => {
    try {
      ...
      toast.error(coverError instanceof Error ? coverError.message : "Não foi possível usar esta capa.")
    }
  }
  ...
  const handlePerVideoCoverChange = (mediaId: string, file: File | null) => {
    try {
      ...
      setPerVideoCovers((current) => {
        ...
        const next = { ...current }
        if (file) { next[mediaId] = { file, preview: URL.createObjectURL(file) } } else { delete next[mediaId] }
        return next
      })
    } catch (coverError) {
      toast.error(coverError instanceof Error ? coverError.message : "Não foi possível usar esta capa.")
    ...
    try {
      ...
      if (videoItems.length > 0 && coverMode !== "none") {
        if (coverMode === "single" && sharedCover) {
          ...
          videoItems.forEach((item) => { itemCovers.push({ mediaId: item.id, coverUrl: sharedUrl }) })
        }
      ...
      const response = await fetch("/api/batches", {
        ...
        body: JSON.stringify({
          ...
          singleCaption,
          singleHashtags,
          itemCaptions: selectedMedia.map((mediaId) => ({ mediaId, caption: perMedia[mediaId]?.caption || "", hashtags: perMedia[mediaId]?.hashtags || "" })),
          rotationCaptions,
          itemCovers,
        }),
      })
    ...
    <div>
      <div className="mb-7 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Monte uma sequência de mídias, legendas, capas e intervalos para publicar automaticamente.</p>
        </div>
        <button
          onClick={() => setRandomMode(!randomMode)}
          className={`flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-colors ${
            randomMode ? "border-green-500/30 bg-green-500/15 text-green-300" : "border-white/10 bg-white/5 text-gray-400 hover:text-white"
          }`}
        >
          <Shuffle size={15} />
            ...
            <>
              <section className="rounded-2xl border border-white/[0.07] bg-[#111] p-5">
                <div className="mb-4 flex items-center justify-between gap-3">
                  ...
                  <button onClick={() => router.push("/dashboard/library")} className="text-xs font-medium text-purple-400 hover:text-purple-300">Abrir biblioteca</button>
                </div>
                ...
                {media.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {visibleMedia.map((item) => {
                      ...
                      return (
                        <button key={item.id} onClick={() => toggleMedia(item.id)} className={`relative overflow-hidden rounded-xl border text-left ${selected ? "border-purple-500/60" : "border-white/[0.07] hover:border-white/20"}`}>
                          ...
                          {selected && (
                            <span className="absolute left-2 top-2 flex h-7 min-w-7 items-center justify-center rounded-lg bg-purple-500 px-2 text-xs font-bold text-white">{position + 1}</span>
                          )}
                        </button>
                      )
                    })}
                ...
                {media.length > 3 && (
                  <button type="button" onClick={() => setShowAllMedia((c) => !c)} className="mt-4 flex w-full items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.025] px-4 py-2.5 text-sm font-medium text-purple-300 transition-colors hover:border-purple-500/30 hover:bg-purple-500/[0.08]">
                  ...
                  <div className="mt-5 space-y-2">
                    ...
                    {selectedItems.map((item, index) => (
                      <div key={item.id} className="flex items-center gap-3 rounded-xl border border-white/[0.07] bg-white/[0.025] p-3">
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-purple-500/15 text-xs font-bold text-purple-300">{index + 1}</span>
                ...
                {rotationCaptions.map((entry, index) => (
                  <div key={index} className="rounded-xl border border-white/[0.07] p-4">
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      {rotationCaptions.length > 1 && <button onClick={() => setRotationCaptions((c) => c.filter((_, i) => i !== index))} className="text-red-400 hover:text-red-300"><Trash2 size={14} /></button>}
                    </div>
                    <CaptionFields value={entry} onChange={(field, value) => updateRotation(index, field, value)} />
                  </div>
                ))}
                <button onClick={() => setRotationCaptions((c) => [...c, { caption: "", hashtags: "" }])} className="flex items-center gap-2 text-sm font-medium text-purple-400 hover:text-purple-300">
